            ...
            if (attention_mask is not None and not attention_mask.all()) or (past_key_values and seq_length != 1):
                ...
                for i in range(len(input_ids)):
                    ...
                    new_attention_mask.append(torch.cat(
                        (attention_mask[i, :boi_token_pos + 1], torch.ones(num_patches).to(attention_mask.device), attention_mask[i, eoi_token_pos:]))) # 在两个token之间加入
                    
                    new_input_ids.append(torch.cat(
                        (input_ids[i, :boi_token_pos + 1], input_ids[i, -1].repeat(num_patches), 
                         input_ids[i, eoi_token_pos:]))) # 在两个token之间加入
        ...
        if labels is not None:
            ...
            for i in range(len(input_ids)):
                ...
                new_labels.append(torch.cat(
                    (labels[i, :boi_token_pos + 1], torch.tensor([-100]).to(labels.device).to(labels.dtype).repeat(1600), labels[i, eoi_token_pos:]))) # 在两个token之间加入            
